...
Creation and takeover remain user-only. Tools accept named values, not command strings. Pod numbers are zero-based; omitted broadcast `pods` means all pods, while an empty list is invalid. A multiline message is a normal string, with no extra command quoting. Tools return an immediate receipt. Successful agent broadcasts and board posts do not generate a second interrupt. Failures, uncertain delivery, and lifecycle completions still produce compact feedback. No polling is required. A completed transport send is not an acknowledgement of peer acceptance or execution. Uncertain sends are never automatically replayed.
